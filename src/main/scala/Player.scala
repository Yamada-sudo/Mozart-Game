  ...
  case Measure(chords) =>
    chords.foreach { chord =>
      chord.notes.foreach { note =>
        context.system.scheduler.scheduleOnce(chord.date.milliseconds, self, MidiNote(note.pitch, note.vol, note.dur, chord.date))
        context.system.scheduler.scheduleOnce((chord.date + note.dur).milliseconds, self, MidiNote(note.pitch, 0, 0, chord.date + note.dur))
      }
    }
